        ...
        flashcard?.let {
            ...
            when (it.type) {
                ...
                FlashcardType.MULTIPLE_CHOICE -> {
                    ...
                    // Parse the options from JSON string to List if available
                    val optionsList = it.options?.let { optionsJson ->
                        try {
                    ...
                    if (optionsList.isNotEmpty()) option1EditText.setText(optionsList.getOrElse(0) { "" })
                    if (optionsList.size > 1) option2EditText.setText(optionsList.getOrElse(1) { "" })
                    if (optionsList.size > 2) option3EditText.setText(optionsList.getOrElse(2) { "" })
                    if (optionsList.size > 3) option4EditText.setText(optionsList.getOrElse(3) { "" })
                ...
                FlashcardType.BASIC -> {
                    flashcardTypeSpinner.setText(flashcardTypes[0], false)  // Tratar BASIC como FRONT_BACK
                    frontEditText.setText(it.front)
                    backEditText.setText(it.back)
                    frontBackLayout.visibility = View.VISIBLE
                }
